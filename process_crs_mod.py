...
for NUMERO in range(1, 20):
    ...
    new_segment = lambda : {
        ...
        "nb_interventions_PLQ":0,
        "nb_interventions_QS":0,
        "nb_interventions_PQ":0
    }
    current = new_segment()
    ...
    def type_intervenant(interv):
        ...
        if interv in ["M. Barrette", "M. Birnbaum", "M. Tanguay", "M. Nadeau-Dubois", "M. Zanetti", "Mme Hivon", "M. Ouellet"]:
            return "opposition"
        if interv in ["M. Caire", "M. Lévesque (Chapleau)", "M. Lemieux", "M. Martel", "M. Lefebvre", "M. Provençal", "M. Poulin", "Mme D'Amours", "Mme Lavallée", "Mme Boutin", "M. Tremblay", "M. Lamothe", "M. Asselin", "M. Caron", "Mme Lachance"]:
            return "gouvernement"
        print("WARNING: un intervenant ne semble pas correct: %s" % interv,file=sys.stderr)
        return ""

    def type_intervenants_precision(interv):
        if interv in ["M. Barrette", "M. Birnbaum", "M. Tanguay"]:
            return "PLQ"
        if interv in ["M. Nadeau-Dubois", "M. Zanetti"]:
            return "QS"
        if interv in ["Mme Hivon", "M. Ouellet"]:
            return "PQ"
    ...
    for line in text.split("\n"):
        ...
        if match_inter and not any(match_inter.group(1).startswith(x) for x in ["La CAI", "La Pinière", "Le Québec", "Le Jeune Barreau"]):
            ...
            if type_intervenants_precision(match_inter.group(1)):
                current["nb_interventions_%s" % type_intervenants_precision(match_inter.group(1))] += 1
